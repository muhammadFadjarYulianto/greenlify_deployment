...
const BlogCard = ({author, content, created_at, img_file, title, views}) => {
    ...
    return (
        <Card className="flex flex-col h-full">
            ...
            <CardFooter
                className="flex flex-col lg:flex-row items-start lg:items-center gap-4 text-emerald-600 mt-auto">
                <div className="flex items-start md:items-center">
                    <Avatar className={`cursor-pointer ${bgColor} text-white shadow-md hidden lg:block`}>
                        <AvatarImage src="" alt="User avatar"/>
                        <AvatarFallback>
                            {author.split(' ').map(word => word[0]).join('').toUpperCase() || <User/>}
                        </AvatarFallback>
                    </Avatar>
                    <Typography variant="p-semibold" className="text-emerald-600 ml-0 lg:ml-4">{author}</Typography>
                </div>
